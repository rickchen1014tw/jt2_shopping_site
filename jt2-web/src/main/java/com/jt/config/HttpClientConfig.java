...
public class HttpClientConfig {
	...
	//定义httpClient链接池管理器   實例化一個bean，並且指定id為httpClientConnectionManager
	 ...
	 * 这里需要以参数形式注入上面实例化的连接池管理器
      @Qualifier 指定bean标签进行注入
             如果沒有用@Qualifier修飾，默認是按類型在容器裡找對象，若有這個類型的對象就注入
             而我們現在用@Qualifier修飾，就能更精確的保證一定是上面生成的實例 
	 */
	 ...
	 */
	@Bean
	@Scope("prototype")	//默認是單例，將來要發起多個請求，設成多例比較好，一個Client只有一個人能用，改成多例，將來誰需要誰就取得一個
